...
/*   Updated: 2024/04/25 16:05:22 by jose-gon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	grade_color(t_map *map, t_point *point)
{
	...
	else
	{
		...
		{
		r = get_r(DEFAULT_GRO) + ((get_r(DEFAULT_TOP) - get_r(DEFAULT_GRO)) * color_steps(map->min_z, map->max_z, point->axis[Z]));
		g = get_g(DEFAULT_GRO) + ((get_g(DEFAULT_TOP) - get_g(DEFAULT_GRO)) * color_steps(map->min_z, map->max_z, point->axis[Z]));
		b = get_b(DEFAULT_GRO) + ((get_b(DEFAULT_TOP) - get_b(DEFAULT_GRO)) * color_steps(map->min_z, map->max_z, point->axis[Z]));
